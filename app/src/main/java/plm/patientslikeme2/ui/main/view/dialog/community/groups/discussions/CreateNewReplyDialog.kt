...
import plm.patientslikeme2.databinding.DialogNewReplyBinding
...
class CreateNewReplyDialog(
    private val commentBody: String,
    private val discussionId: String,
    private val commentId: String,
    private val replyTo: String,
    private val shouldRefresh: () -> Unit
) : BaseBottomSheetFragment<DialogNewReplyBinding>() {
    ...
    ): DialogNewReplyBinding = DialogNewReplyBinding.inflate(inflater, container, false)
    ...
    private fun initView() {
        binding?.title?.text = getString(R.string.reply_to, replyTo)
        binding?.tvDiscussionHeader?.text = Jsoup.parse(commentBody).text()
        setListeners()
    }
    ...
    private fun createComment() {
        binding?.editor?.htmlText?.getHtml()?.let {
            viewModel.addReplyOnComments(discussionId, commentId, it).observe(viewLifecycleOwner) {
                when {
                    it.status.isLoading() -> {
                        showProgress()
                    }
