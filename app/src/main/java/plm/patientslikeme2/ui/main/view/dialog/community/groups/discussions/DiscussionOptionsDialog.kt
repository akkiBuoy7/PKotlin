...
class DiscussionOptionsDialog(private val discussionId: Int, private val userId: Int, private val shouldRefresh: () -> Unit) : BaseBottomSheetFragment<DialogDiscussionOptionBinding>() {
    ...
    private fun setListeners() {
        ...
        binding?.tvFlag?.setOnClickListener {
            viewModel.flagPost(discussionId).observe(viewLifecycleOwner) {
                when {
                    it.status.isLoading() -> {
                        showProgress()
                    }
